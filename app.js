...
// ルート順序と所要時間（始発からの累積分数）
const ROUTE_ORDER = {
  north: {
    toDeck: {
      stops: ["cs", "jp", "westcart", "wtw", "fj", "uwl", "deck"],
      offsets: [0, 1, 2, 4, 6, 7, 9]
    },
    toCS: {
      stops: ["deck", "uwl", "fj", "wtw", "westcart", "jp", "cs"],
      offsets: [0, 2, 3, 5, 7, 8, 9]
    }
  },
  south: {
    toDeck: {
      stops: ["cs", "parade", "gate4", "sfr", "stage33", "deck"],
      offsets: [0, 1, 2, 3, 4, 5]
    },
    toCS: {
      stops: ["deck", "stage33", "sfr", "gate4", "parade", "cs"],
      offsets: [0, 1, 2, 3, 4, 5]
    }
  }
};

// 共有停留所のTIMETABLEキーをルート別に解決
function getTimetableKey(stopId, route) {
  if (stopId === 'cs') return route === 'south' ? 'cs_south' : 'cs_north';
  if (stopId === 'deck') return route === 'south' ? 'deck_south' : 'deck_north';
  return stopId;
}
...
// ===== 運休判定 =====
// 所要時間オフセットを使って始発時刻を逆算し、運休キーと照合する
function isSuspended(stopId, route, direction, hour, minute) {
  const routeData = ROUTE_ORDER[route][direction];
  const stopIndex = routeData.stops.indexOf(stopId);
  if (stopIndex < 0) return false;

  const stopOffset = routeData.offsets[stopIndex]; // この停留所の始発からの所要時間（分）

  // この停留所の時刻から始発時刻を逆算
  const totalMinutes = hour * 60 + minute - stopOffset;
  const startHour = Math.floor(totalMinutes / 60);
  const startMinute = totalMinutes % 60;

  const key = `${route}_${direction}_${startHour}:${String(startMinute).padStart(2, '0')}`;
  return !!state.suspendedTimes[key];
}

// ===== 時刻表取得 =====
function getSchedule(stopId, route, direction, mode) {
  const tKey = getTimetableKey(stopId, route);
  const stopData = TIMETABLE[tKey];
  if (!stopData) return [];
...
function updateBusInfo() {
  if (!state.currentStopId || !state.currentDirection) {
    ...
    document.getElementById('busPositionTrack').innerHTML = '';
    return;
  }
...
function updateBusPosition() {
  const track = document.getElementById('busPositionTrack');
  if (!track) return;
  track.innerHTML = '';

  if (!state.currentStopId || !state.currentRoute || !state.currentDirection) return;

  const route = state.currentRoute;
  const direction = state.currentDirection;
  const routeData = ROUTE_ORDER[route][direction];
  const stops = routeData.stops;
  const offsets = routeData.offsets;

  // 選択中のバス停のルート内インデックスを取得
  const selectedStopIdx = stops.indexOf(state.currentStopId);
  if (selectedStopIdx < 0) return;
  const selectedStopOffset = offsets[selectedStopIdx] * 60; // 秒単位

  // 始発停留所の時刻表を取得
  const startStopId = stops[0];
  const schedule = getSchedule(startStopId, route, direction, state.closingMode);
  if (!schedule || schedule.length === 0) return;

  const nowSec = nowSeconds();
  const totalRouteTime = offsets[offsets.length - 1] * 60;

  // 選択バス停に最も近いバスを探す
  // 優先1: 既に出発していて、まだ選択バス停に到着していないバス（向かっている）
  // 優先2: 選択バス停を通過したばかりで、まだルート上にいるバス（通過直後）
  let targetBusStartSec = null;
  let targetBusElapsed = null;

  // まず未着のバスを探す（順方向検索で最初にヒットしたものが最も近い）
  for (const timeStr of schedule) {
    const [h, m] = timeStr.split(':').map(Number);
    const startSec = h * 3600 + m * 60;
    const arriveAtSelectedSec = startSec + selectedStopOffset;

    // 既に出発していて、まだ選択バス停に到着していない
    if (nowSec >= startSec && nowSec < arriveAtSelectedSec) {
      targetBusStartSec = startSec;
      targetBusElapsed = nowSec - startSec;
      break;
    }
  }

  // 未着のバスがない場合、通過直後のバスを探す
  if (targetBusStartSec === null) {
    for (let i = schedule.length - 1; i >= 0; i--) {
      const [h, m] = schedule[i].split(':').map(Number);
      const startSec = h * 3600 + m * 60;
      const endSec = startSec + totalRouteTime;
      const arriveAtSelectedSec = startSec + selectedStopOffset;

      // 選択バス停を通過済みだが、まだルート上にいる
      if (nowSec >= arriveAtSelectedSec && nowSec <= endSec) {
        targetBusStartSec = startSec;
        targetBusElapsed = nowSec - startSec;
        break;
      }
  ...
  // それでも見つからない場合、次に出発するバスを待つ
  if (targetBusStartSec === null) {
    const nextBusTime = schedule.find(t => {
      const [h, m] = t.split(':').map(Number);
      return h * 3600 + m * 60 > nowSec;
    });
    if (!nextBusTime) return;
    targetBusElapsed = -1; // 始発待ち
  }

  // バスの現在位置を計算
  let busPositionIndex = 0;
  if (targetBusElapsed > 0) {
    for (let i = offsets.length - 1; i >= 0; i--) {
      if (targetBusElapsed >= offsets[i] * 60) {
        busPositionIndex = i;
        break;
      }
    }
  ...
  // 進行方向を判定（toDeck=右向き、toCS=左向き）
  const goingRight = (direction === 'toDeck');
  const busClass = goingRight ? 'bus-pos-bus moving-right' : 'bus-pos-bus moving-left';
  ...
  if (targetBusElapsed < 0) {
    ...
    if (goingRight) {
      ...
      dashEl.className = 'bus-pos-dash flip';
      dashEl.textContent = '\u{1F4A8}';
      track.appendChild(dashEl);

      const busEl = document.createElement('span');
      busEl.className = busClass;
      busEl.textContent = '\u{1F68C}';
      track.appendChild(busEl);

      const arrow = document.createElement('span');
      arrow.className = 'bus-pos-arrow';
      arrow.textContent = '\u25B6';
      track.appendChild(arrow);

      const stopEl = document.createElement('span');
      stopEl.className = 'bus-pos-stop active';
      stopEl.textContent = getStopShortName(stops[0], route);
      ...
      const dashEl = document.createElement('span');
      dashEl.className = 'bus-pos-dash';
      dashEl.textContent = '\u{1F4A8}';
      track.appendChild(dashEl);
    }
  } else {
    // 走行中: 通過済み停留所 → バス → 次の停留所
    const prevStop = stops[busPositionIndex];
    const nextStop = stops[busPositionIndex + 1];

    if (goingRight) {
      // 右向き: 通過済み ▶ 💨🚌 ▶ 次の停留所
      if (prevStop) {
        const stopEl = document.createElement('span');
        stopEl.className = 'bus-pos-stop passed';
        stopEl.textContent = getStopShortName(prevStop, route);
        track.appendChild(stopEl);

        const arrow1 = document.createElement('span');
        arrow1.className = 'bus-pos-arrow';
        arrow1.textContent = '\u25B6';
        track.appendChild(arrow1);
      }
      ...
      if (nextStop) {
        ...
        arrow2.className = 'bus-pos-arrow';
        arrow2.textContent = '\u25B6';
        track.appendChild(arrow2);

        const stopEl = document.createElement('span');
        stopEl.className = 'bus-pos-stop active';
        stopEl.textContent = getStopShortName(nextStop, route);
        track.appendChild(stopEl);
      }
    } else {
      // 左向き: 次の停留所 ◀ 🚌💨 ◀ 通過済み
...
function getStopShortName(stopId, route) {
...
function renderSuspendTimes() {
  ...
  // 始発停留所の時刻を取得
  const routeData = ROUTE_ORDER[route][direction];
  const startStopId = routeData.stops[0];
  const startStopName = STOPS_DATA[route].find(s => s.id === startStopId)?.name || startStopId;

  const tKey = getTimetableKey(startStopId, route);
  const startData = TIMETABLE[tKey];
  if (!startData) return;

  // 基本ダイヤ（18時）の時刻を取得
  const times = [];
  const baseData = startData['18'];
  if (baseData && baseData[direction]) {
    const baseHours = baseData[direction];
    for (const h of Object.keys(baseHours).sort((a,b) => Number(a) - Number(b))) {
      for (const m of baseHours[h]) {
        times.push({ hour: parseInt(h), minute: m });
  ...
  // 現在の閉園モードが18時以外の場合、延長ダイヤの時刻も追加
  const mode = state.closingMode;
  if (mode !== '18') {
    const extData = startData[mode];
    if (extData && extData[direction]) {
      const extHours = extData[direction];
      for (const h of Object.keys(extHours).sort((a,b) => Number(a) - Number(b))) {
        for (const m of extHours[h]) {
